...
- **Write the first user message immediately after spawn.** Empirically the CLI only emits its `system/init` event once a prompt is waiting on stdin — gating on init first deadlocks. The process stays alive across turns (multi-turn = one process, same `session_id`); `Stop` kills it and emits `stopped` (the `{"type":"interrupt"}` control message is not reliable).
