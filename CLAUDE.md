...
- **Chat settings modal** opens from the gear as the first flex child of `.chat-input` (ChatPanel). `ChatSettingsModal.tsx` has three tabs: Settings (model/max-turns/plan toggle + `sessionUsage` cumulative tokens), History, MCP. History and MCP lists load via IPC into **local component state** — never a zustand selector that returns a fresh array (a `.filter()` in a selector caused "Maximum update depth exceeded" / white screen once).
